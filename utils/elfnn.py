...
class ELFNN:
    ...
    def plot_threshold_stats(self):
        ...
        ax2 = ax1.twinx()
        ax1.errorbar(threshold, n_iter.mean(axis=-1), yerr=n_iter.std(axis=-1), color=self.dmap(0))
        ax1.scatter(threshold, n_iter.mean(axis=-1), color=self.dmap(0))
        ax2.errorbar(threshold, f_labeled.mean(axis=-1), yerr=f_labeled.std(axis=-1), color=self.dmap(3))
        ax2.scatter(threshold, f_labeled.mean(axis=-1), color=self.dmap(3))
        ax1.set_xlabel('Threshold')
        ax1.set_ylabel('Number of iterations', color=self.dmap(0))
        ax2.set_ylabel('Fraction of samples labeled', color=self.dmap(3), labelpad=16)
        ...
        ax1.tick_params(axis='y', colors=self.dmap(0))
        ax2.tick_params(axis='y', colors=self.dmap(3))

        fig, ax = plt.subplots(figsize=(7,3))
        ax.errorbar(threshold, acc_train.mean(axis=-1), yerr=acc_train.std(axis=-1), color=self.dmap(0))
        ax.scatter(threshold, acc_train.mean(axis=-1), color=self.dmap(0), label='Train')
        ax.errorbar(threshold, acc_test.mean(axis=-1), yerr=acc_test.std(axis=-1), color=self.dmap(3))
        ax.scatter(threshold, acc_test.mean(axis=-1), color=self.dmap(3), label='Test')
